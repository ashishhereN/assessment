...
import com.reliaquest.server.controller.MockEmployeeControllerAdvice;
...
public class MockEmployeeService {
	...
	private static final Logger logger = LoggerFactory.getLogger(MockEmployeeService.class);
    ...
    public MockEmployee create(@NonNull CreateMockEmployeeInput input) {
        ...
        logger.debug("Added employee: {}", mockEmployee);
        return mockEmployee;
    }
    ...
    public boolean delete(@NonNull DeleteMockEmployeeInput input) {
        ...
        if (mockEmployee.isPresent()) {
            ...
            logger.debug("Removed employee: {}", mockEmployee.get());
            return true;
        }
